...
class MatchingHead:
    ...
    def forward(self, backbone_output: np.ndarray) -> np.ndarray:
        h = relu(self.layer.forward(backbone_output))
        return l2_normalize(h)

    def backward(self, grad: np.ndarray, lr: float = 0.001) -> np.ndarray:
        """Backward pass. grad comes from contrastive loss."""
        grad_h = grad * relu_derivative(self.layer._input if hasattr(self.layer, '_input') else grad)
